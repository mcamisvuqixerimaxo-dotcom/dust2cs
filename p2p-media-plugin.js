    ...
    negotiate() {
      ...
      return this.serial(async () => {
        if (this.pc.signalingState === 'have-local-offer') {
          // A roster race or a transient signaling loss must not strand an offer.
          this.s.signal({description:{type:'offer',sdp:this.pc.localDescription.sdp}});return;
        }
    ...
    receive(message) {
      return this.serial(async () => {
        if (message.description) {
          ...
          if(d.type==='answer'&&this.pc.signalingState==='stable')return;
